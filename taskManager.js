function moveCompletedTasks() {
    ...
    var calendarId = sheetSetup.getRange("C10").getValue();
    var noWhatsApp = sheetSetup.getRange("C3").getValue(); // Ambil nomor WhatsApp dari setup
    ...
    for (var i = 0; i < valuesReminder.length; i++) {
        if (valuesReminder[i][0] === true) {
            ...
            if (calendarId) {
                ...
                var taskId = "TASK_" + (i + startRow); // ID unik untuk setiap tugas
    ...
    if (newDoneData.length > 0) {
        ...
        // Kirim notifikasi WhatsApp satu kali untuk semua tugas yang selesai
        if (completedTaskMessages.length > 0 && noWhatsApp) {
...
function sortTasksByDeadline() {
    ...
    var formulas = sheet.getRange(startRow, 5, endRow - startRow + 1, 1).getFormulas();

    var filteredValues = values.filter(row => row[1] && row[2]);

    filteredValues.sort((a, b) => new Date(a[2]) - new Date(b[2]));

    var newData = new Array(values.length).fill().map(() => ["", "", "", ""]);
    for (var i = 0; i < filteredValues.length; i++) {
        newData[i] = [filteredValues[i][0], filteredValues[i][1], filteredValues[i][2], filteredValues[i][3]];
    }
    dataRange.setValues(newData);
    sheet.getRange(startRow, 5, endRow - startRow + 1, 1).setFormulas(formulas);
}

function moveTaskBackToReminder(doneRow) {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetDone = ss.getSheetByName("DONE");
    var sheetReminder = ss.getSheetByName("REMINDER");
    var sheetSetup = ss.getSheetByName("SETUP");
